/*
 * Authored by Alex Schnier Hansen
 * For PROG1180-03 Project
 * Last Modified October 13th, 2022
 */
...
function searchProducts(UPC, status, name, details, manFactID, serialNum) {
    ...
    for (product of products) {
        if (
            (UPC == null       || product["UPC"].ToString().includes(UPC.ToString())) &&
            (status == null    || status == product["Status"]) &&
            (name == null      || product["Name"].ToUpper().includes(name.ToUpper())) &&
            (details == null   || product["Details"].ToUpper().includes(details.ToUpper())) &&
            (manFactID == null || manFactID == product["ManFactID"]) &&
            (serialNum == null || product["SerialNum"].ToString().includes(serialNum.ToString()))
        ) { output.push(product) }
    }
...
function searchInventory(invoiceID, UPC, dateReceived, cost, qtyOnHand, invoiceQTY) {
    ...
    for (item of onHandInv) {
        ...
        if (
            (invoiceID == null    || item["InvoiceID"].ToString().includes(invoiceID.ToString())) &&
            (UPC == null          || item["UPC"].ToString().includes(UPC.ToString())) &&
...
function searchManufacturers(manFactID, name, details) {
    ...
    for (manufacturer of manufacturers) {
        if (
            (manFactID == null || manufacturer["ManFactID"].ToString().includes(manFactID.ToString())) &&
            (name == null      || manufacturer["Name"].ToUpper().includes(name.ToUpper())) &&
            (details == null   || manufacturer["Details"].ToUpper().includes(details.ToUpper()))
...
function searchInvoices(invoiceID,orderDate,altSysCode,customerID,equipmentID) {
    ...
    for (invoice of invoices) {
        if (
            (invoiceID == null   || invoice["InvoiceID"].ToString().includes(invoiceID.ToString())) &&
            (orderDate == null   || orderDate == invoice["OrderDate"]) &&
            (altSysCode == null  || invoice["AltSysCode"].ToString().includes(altSysCode.ToString())) &&
            (customerID == null  || invoice["CustomerID"].ToString().includes(customerID.ToString())) &&
            (equipmentID == null || invoice["EquipmentID"].ToString().includes(equipment.ToString()))
        ) { output.push(invoice) }
    }
...
function searchCustomers(customerID, fName, lName, street, city, postCode, province, phone) {
    ...
    for (customer of customers) {
        if (
            (customerID == null || invoice["CustomerID"].ToString().includes(customerID.ToString())) &&
            (fName == null      || invoice["FName"].ToUpper().includes(fName.ToUpper())) &&
            (lName == null      || invoice["LName"].ToUpper().includes(lName.ToUpper())) &&
            (street == null     || invoice["Street"].ToUpper().includes(street.ToUpper())) &&
            (city == null       || invoice["City"].ToUpper().includes(city.ToUpper())) &&
            (postCode == null   || invoice["PostCode"].ToUpper().includes(postCode.ToUpper())) &&
            (province == null   || province == invoice["Province"]) &&
            (phone == null      || invoice["Phone"].ToString().includes(phone.ToString()))
 ...
 * ORDER INTERACTIONS *
 *********************/
 function searchOrders(orderID, customerID, itemUPC, orderDate, placeholder1, placeholder2) {
    // pull down orders array and initialize output array
    let orders = JSON.parse("JSON STRING")
    let output = []

    // if order matches all provided criteria, add it to output
    for (order of orders) {
        if (
                (orderID == null      || order["OrderID"].ToString().includes(orderID.ToString())) &&
                (customerID == null   || order["CustomerID"].ToString().includes(customerID.ToString())) &&
                (itemUPC == null      || order["ItemUPC"].ToString().includes(itemUPC.ToString())) &&
                (orderDate == null    || order["OrderDate"] == orderDate) &&
                (placeholder1 == null || order["Placeholder1"] == placeholder1) &&
                (placeholder2 == null || order["Placeholder2"] == placeholder2) 
            ) { output.push(order) }
    }
...
function addOrder(orderID, customerID, itemUPC, orderDate, placeholder1, placeholder2) {
    let funcStatus = "Failed to add order."

    // pull down array of order
    let orders = JSON.parse("JSON STRING")

    // check for duplicate order
    if (orders.getIndex(order => (order["CustomerID"] === customerID) && (order["ItemUPC"] === itemUPC) && (order["OrderDate"] == orderDate) != -1)) {
        let funcStatus = "This order is already in the system."
        return funcStatus
    }
    ...
    // idk man make Nic do it lol
    // if (success) {
        funcStatus = "Order added successfully."
    // }
    return funcStatus
}

function editOrder(orderID, customerID, itemUPC, orderDate, placeholder1, placeholder2) {
    let funcStatus = "Failed to edit order."
    // pull down array
    let orders = JSON.parse("JSON STRING")

    // check for missing order in array and store index for slice
    let index = orders.findIndex(order => order["ID"] === orderID)
    if (index === -1) {
        funcStatus = "The order you are trying to edit does not exist."
        return funcStatus
    }

    // slice order from array
    let order = orders.slice(index, index + 1)

    // update order, ignoring any null inputs
    order["CustomerID"] = customerID
    order["ItemUPC"] = itemUPC
    order["OrderDate"] = orderDate
    order["Placeholder1"] = placeholder1
    order["Placeholder2"] = placeholder2

    // return array to JSON
    // idk man make Nic do it lol
    // if (success) {
    funcStatus = "Order updated successfully."
    // }
    return funcStatus
}

function deleteOrder(orderID) {
    let funcStatus = "Failed to delete order."
    // pull down arrays
    let orders = JSON.parse("JSON STRING")

    // ensure record exists and store index for splice
    let index = orders.findIndex(order => order["OrderID"] === orderID)
    if (index === -1) {
        funcStatus = "The order you are trying to delete does not exist."
        return funcStatus
    }
    ...
    // idk man make Nic do it lol
    // if (success) {
    funcStatus = "Order deleted successfully."
    // }
    return funcStatus
}
